...
class BaseTrainingRecipe:
    ...
    def training_model_converse(self, model):
        if self.training_args.tune_type_llm == "lora":
            lora_config = LoraConfig(
                r=self.training_args.llm_lora_r,
                lora_alpha=self.training_args.llm_lora_alpha,
                lora_dropout=self.training_args.llm_lora_dropout,
                bias=self.training_args.llm_lora_bias,
                task_type="CAUSAL_LM",
            )
    ...
    def _llm_tune_type_setting(self, model):
        tune_type = self.training_args.tune_type_llm.lower()
        if tune_type == "full":
        ...
        self.support_gradient_checkpoint(model.llm, self.training_args.gradient_checkpointing)

        return model

    def _encoder_image_tune_type_setting(self, model):
        tune_type = self.training_args.tune_type_encoder_image.lower()
        if tune_type == "full":
    ...
    def _connector_image_tune_type_setting(self, model):
        tune_type = self.training_args.tune_type_connector_image.lower()
        if tune_type == "full":
    ...
    def _encoder_image3d_tune_type_setting(self, model):
        tune_type = self.training_args.tune_type_encoder_image3d.lower()
        if tune_type == "full":
    ...
    def _connector_image3d_tune_type_setting(self, model):
        tune_type = self.training_args.tune_type_connector_image3d.lower()
        if tune_type == "full":
