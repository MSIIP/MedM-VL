...
CUDA_VISIBLE_DEVICES=0 python lvlm/inference.py \
    ...
    --data_path /hdd/shiym/datasets_processed/MedM-VL/ctrate/valid_choice_dialog.json \
    --conv_version qwen2 \
...
CUDA_VISIBLE_DEVICES=0 python scripts/eval/ctrate/eval_ctrate_text.py \
    --answer_path /hdd/shiym/datasets_processed/MedM-VL/ctrate/valid_choice_dialog.json \
    --predict_path $MODEL_PATH/eval/valid_choice_dialog.json \
    --result_path $MODEL_PATH/eval/valid_choice_dialog_result.json

python scripts/eval/ctrate/eval_ctrate_choice.py \
    --answer_path /hdd/shiym/datasets_processed/MedM-VL/ctrate/valid_choice_dialog.json \
    --predict_path $MODEL_PATH/eval/valid_choice_dialog.json \
    --result_path $MODEL_PATH/eval/valid_choice_dialog_result_acc.json
