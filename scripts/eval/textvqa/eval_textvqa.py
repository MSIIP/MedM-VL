...
def main(args):
    ...
    answers = {(answer["image_id"], answer["question"].lower()): answer for answer in answers}

    textvqa_results = []
    for question_dict, predict in tqdm(zip(question_list, predict_list), total=len(question_list)):
        image_id = question_dict["id"]
        question = prompt_processor(question_dict["conversations"][0]["value"][8:])
        gt_answers = answers[(image_id, question)]["answers"]

        pred_answer = predict.strip()
        textvqa_results.append(dict(gt_answers=gt_answers, pred_answer=pred_answer))

    evaluator = TextVQAAccuracyEvaluator()
    print('Samples: {}\nAccuracy: {:.2f}%\n'.format(len(textvqa_results), 100. * evaluator.eval_pred_list(textvqa_results)))
